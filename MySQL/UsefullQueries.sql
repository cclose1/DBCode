...
SELECT 
	*,
    Charge / PercentGain                     AS ChargePerGain,
    60 * percentGain / ChargeDurationMinutes AS chPerHr,
    ChargeDurationMinutes / PercentGain      AS MinPerPercent,
    ChargeDurationMinutes - EstDuration      AS EstDifference
FROM Expenditure.SessionChargeDetails
...
INSERT INTO Expenditure.smartmeterusagedata(Timestamp, Type, Reading, Comment)
SELECT 
    OC.End AS Timestamp,
    OC.Type,
    CASE
		WHEN OC.Type = 'Gas' THEN  KwhToUnitsByDate(OC.Reading, OC.Start)
        ELSE OC.Reading
	END AS Reading,
	'OctEnergy Data' AS Commnet
    FROM expenditure.octensmdata OC
